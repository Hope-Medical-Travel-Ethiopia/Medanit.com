...
const Services = () => {
  ...
  return (
    <>
      <div
        className="Services w-100 text-[#333] lg:p-20 p-5 lg:px-40 z-0 lg:pb-0 pb-0"
        id="services"
      >
        {/*  */}
        <div className="flex flex-col items-center w-[90%] lg:w-[60%] text-center justify-center m-auto">
          <h1
            className="text-5xl uppercase font-black tracking-widest leading-normal 
          text-transparent bg-clip-text bg-gradient-to-br from-blue-500 
          via-sky-600 to-cyan-500"
          >
            {t.nav.service}
          </h1>
          <p className="text-gray-600 text-sm">{t.serviceDetail}</p>
        </div>
        <div>
...
const ServiceComponent = ({ title, description, picture, id }) => {
  ...
  return (
    <section
      className={`flex flex-wrap lg:flex-nowrap text-left items-center my-20 lg:my-20 justify-center lg:justify-between gap-10 lg:gap-20  ${
    ...
    >
      ...
      <div className=" flex items-center ">
        <div className="servieDesc md:px-48 lg:p-0 p-0 w-full justify-self-start">
          ...
          <p className="serviceDesc text-base py-5">{description}</p>
          <Link href="#Header">
